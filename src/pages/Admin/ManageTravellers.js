...
const ManageTraveller = () => {
  ...
  const handleAddTraveler = async () => {
    ...
    try {
      ...
      if (error.response && error.response.data && error.response.data.errors) {
        // If the error response has an errors property, which contains an array of error messages:
        const errorMessage = error.response.data.errors.join('\n');
        alert(errorMessage); // Display the error message as an alert to the user
      } else {
        alert('email already taken'); // Otherwise, display a generic error message
      }
    }
  };
  ...
  const handleUpdateTraveler = async () => {
    try {
      const traveler = { id: formData.id, name, email, password, phone, type };
